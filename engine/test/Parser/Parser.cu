...
void test_parse(std::string input, std::vector<Sym::Symbol> expected_result) {
    auto result = parse_function(input);
...
void test_parser_error(std::string input) {
    EXPECT_THROW(parse_function(input), std::invalid_argument);
}
...
PARSER_TEST(LogarithmWithArbitraryBase2, "log_sin(pi)(x)", Sym::log(Sym::sin(Sym::pi()), Sym::var()))
...
PARSER_TEST(BracesOverFunctionCall, "ln(x-3*5)", Sym::ln(Sym::var() - Sym::num(3) * Sym::num(5)))
PARSER_TEST(BracesOverLogarithmBase, "log_(x+1)(e)", Sym::log(Sym::var() + Sym::num(1), Sym::e()))

PARSER_TEST(LeftAssociativityOfAddition, "x+1+e+pi", Sym::var() + Sym::num(1) + Sym::e() + Sym::pi())
PARSER_TEST(LeftAssociativityOfMultiplication, "x*1*e*pi", Sym::var() * Sym::num(1) * Sym::e() * Sym::pi())
PARSER_TEST(LeftAssociativityOfSutraction, "x-1-e-pi", Sym::var() - Sym::num(1) - Sym::e() - Sym::pi())
PARSER_TEST(LeftAssociativityOfDivision, "x/1/e/pi", Sym::var() / Sym::num(1) / Sym::e() / Sym::pi())

PARSER_TEST(RightAssociativityOfPower, "e^2^pi^x", Sym::e() ^ (Sym::num(2) ^ (Sym::pi() ^ Sym::var())))
...
PARSER_TEST(TrimSpaces, "  2 +    3  * sin   (  c )       ", Sym::num(2)+Sym::num(3)*Sym::sin(Sym::cnst("c")))
PARSER_TEST_ERR(DoesNotRecognizeFunctionNameSplitWithSpaces, "t g(x)")

// parser skips integral token
PARSER_TEST(IntWithDifferential, "int x^2 dx", Sym::var() ^ Sym::num(2))
PARSER_TEST(IntWithoutDifferential, "int x^2", Sym::var() ^ Sym::num(2))
PARSER_TEST(IntegralWithDifferential, "integral x^2 dx", Sym::var() ^ Sym::num(2))
PARSER_TEST(IntegralWithoutDifferential, "integral x^2", Sym::var() ^ Sym::num(2))
PARSER_TEST(IntegrateWithDifferential, "integrate x^2 dx", Sym::var() ^ Sym::num(2))
PARSER_TEST(IntegrateWithoutDifferential, "integrate x^2", Sym::var() ^ Sym::num(2))
