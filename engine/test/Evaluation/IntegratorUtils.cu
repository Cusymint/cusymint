...
namespace Test {
    ...
    Sym::ExpressionArray<Sym::SubexpressionCandidate> with_count(const size_t count) {
        return Sym::ExpressionArray<Sym::SubexpressionCandidate>(Sym::EXPRESSION_MAX_SYMBOL_COUNT,
                                                                 Sym::MAX_EXPRESSION_COUNT, count);
    }
    ...
    SymVector nth_expression_candidate(size_t n, const SymVector& child, size_t vacancy_idx) {
        ...
        candidate[0].as<Sym::SubexpressionCandidate>().subexpressions_left = subexpressions_left;
        return candidate;
    }
