...
namespace Sym::KnownIntegral {
    __device__ size_t is_power_function(const Integral& integral) {
        return Pow<Var, AllOf<AlmostConst, Not<Integer<-1>>>>::match(integral.integrand()) ? 1 : 0;
    }

    __device__ EvaluationStatus
    integrate_power_function(const Integral& integral, SymbolIterator& destination,
    ...
    __device__ EvaluationStatus
    integrate_reciprocal(const Integral& integral, SymbolIterator& destination,
                         const ExpressionArray<>::Iterator& /*help_space*/) {
